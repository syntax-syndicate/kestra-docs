...
The `build.gradle` handle most of Kestra use case by default using:

* `annotationProcessor group: "io.kestra", name: "processor", version: kestraVersion` for generating plugin metadata.
...
You can read any files from Kestra storage using the method `runContext.storage().getFile()`
```java
...
Don't forgot to provide Outputs with the link generated by `putFile` in order to be usable by others tasks.
...
You can expose metrics to add observability to your task. Metrics will be recorded with the execution and can be accessed via the UI or as [Prometheus metrics](../09.administrator-guide/03.monitoring/index.md#prometheus).
...
Here is how you can develop a [Trigger](../06.workflow-components/05.triggers/index.md).
...
### Documentation
Remember to document your triggers. For this, we provide a set of annotations explained in the [Document each plugin](./07.documentation.md) section.

## Develop a Condition

Here is how you can develop a new [Condition](../06.workflow-components/05.triggers/conditions.md).
